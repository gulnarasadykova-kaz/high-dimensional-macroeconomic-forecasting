# =============================================================================
# Comprehensive Rolling Window Forecasting: RW, PCR, DFM, Ridge, Lasso, Elastic Net,
#                             3PRF, PCLAR, PCLAS
#
...
non_numeric_cols <- names(data)[!sapply(data, function(x) is.numeric(x) || is.integer(x))]

if (length(non_numeric_cols) > 0) {
  data_numeric <- data[, !(names(data) %in% non_numeric_cols)]
  cat("Removed non-numeric columns:\n   ", paste(non_numeric_cols, collapse = ", "), "\n")
  cat(sprintf("New dimensions: %d rows x %d columns\n", nrow(data_numeric), ncol(data_numeric)))
} else {
  data_numeric <- data
  cat("All columns are already numeric (int or num).\n")
}

cat(sprintf("Current data structure: %s\n", class(data_numeric)))

#Convert to Numeric Matrix & Check NAs
my_data_matrix <- as.matrix(data_numeric)
cat("Data converted to numeric matrix (my_data_matrix).\n")

if (anyNA(my_data_matrix)) {
  num_rows_before <- nrow(my_data_matrix)
  rows_with_na <- sum(rowSums(is.na(my_data_matrix)) > 0)
  cat(sprintf("Missing values detected in %d of %d rows.\n", rows_with_na, num_rows_before))
  ...
  cat(sprintf("Removed %d rows. Remaining observations: %d\n", 
              num_rows_before - num_rows_after, num_rows_after))
} else {
  my_data_matrix_cleaned <- my_data_matrix
  cat("No missing values detected. Proceeding with full dataset.\n")
}

#STEP 1: Perfect Multicollinearity Check 
cat("Step 1: Perfect Multicollinearity Assessment")

strict_rank <- rankMatrix(my_data_matrix_cleaned, tol = .Machine$double.eps)[1]
num_variables <- ncol(my_data_matrix_cleaned)

cat(sprintf("Total Variables (p): %d\n", num_variables))
cat(sprintf("Matrix Rank (R): %d (tolerance = %.1e)\n", strict_rank, .Machine$double.eps))

if (strict_rank < num_variables) {
  cat("Perfect multicollinearity detected (R < p) at strict tolerance.\n")
  cat(sprintf("Interpretation: %d variables are perfectly collinear.\n", num_variables - strict_rank))
} else {
  cat("No perfect multicollinearity detected at machine precision.\n")
  cat("Proceeding to check for near-perfect redundancy (numerical dependencies)...\n")
}

#STEP 2: Near-Perfect Redundancy via QR Decomposition 
cat("Step 2: Near-Perfect Redundancy Identification")
...
# Identify retained and redundant variables
...
redundant_variables <- colnames(my_data_matrix_cleaned)[redundant_indices]

#Results Summary 
cat(sprintf("Tolerance level: %.1e\n", TOLERANCE))
cat(sprintf("Observed Rank (R): %d / %d variables\n", observed_rank, num_vars))
cat(sprintf("Redundant (near-collinear) variables detected: %d\n\n", num_vars - observed_rank))

if (length(redundant_variables) > 0) {
  cat("Variables to DROP (linearly dependent / near-redundant):\n")
  cat(paste(redundant_variables, collapse = ", "), "\n\n")
} else {
  cat("None — all variables are linearly independent at this tolerance level.\n\n")
}
...
data <- data %>% 
  dplyr::select(-c("X_FRED_5...39", "X_FRED_5...40", "X_FRED_4...53", "X_FRED_4...58"))
...
p_Y <- ggplot(data.frame(Date = month, Y = Y), aes(x = Date, y = Y)) +
  ...
  labs(subtitle = "Monthly time series of crude oil price (WTI) changes", # WTI Oil Price (1984–2019)
...
#PREPARE DATA FOR 3PRF (needs lagged Y) - UPDATED
...
for (t_start in 1:n_predictions) {
  ...
  #Prepare training and test data (raw)
  X_train_raw <- X[t_start:t_end, ]
  Y_train <- Y[t_start:t_end]
  X_next_raw <- X[t_end + 1, , drop = FALSE]
  
  #Standardize predictors using training sample only 
  ...
  # PCA setup with Bai & Ng IC (for PCR and DFM) 
